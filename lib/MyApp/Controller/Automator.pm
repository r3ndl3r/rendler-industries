# /lib/MyApp/Controller/Automator.pm

package MyApp::Controller::Automator;
...
sub _trigger_automated_notifications {
    ...
    my $message = "$emoji **Scheduled Task Completed**\n\n"
                ...
                . "[View Log](" . $app->config->{url} . "/automator)";
    ...
    foreach my $n (@$notifs) {
        ...
        if ($n->{channel} eq 'discord') {
            ...
            $c->push_fcm($n->{user_id}, $subject, $message, "/automator", "automator_$history_id");
        }
...
sub register_routes {
    my ($class, $bridges) = @_;
    my $r = $bridges->{admin};
    $r->get('/automator')->to('automator#index');
    $r->get('/automator/api/status')->to('automator#api_vault_status');
    $r->get('/automator/api/state')->to('automator#api_state');
    $r->get('/automator/api/history')->to('automator#api_history');
    $r->post('/automator/api/run')->to('automator#api_run');
    $r->post('/automator/api/abort/all')->to('automator#api_abort_all');
    $r->post('/automator/api/abort/:id')->to('automator#api_abort');
    $r->post('/automator/api/inventory/save')->to('automator#api_save_inventory');
    $r->post('/automator/api/inventory/delete/:id')->to('automator#api_delete_inventory');
    $r->post('/automator/api/playbook/save')->to('automator#api_save_playbook');
    $r->post('/automator/api/playbook/delete/:id')->to('automator#api_delete_playbook');
    $r->post('/automator/api/secret/save')->to('automator#api_save_secret');
    $r->post('/automator/api/vault/setup')->to('automator#api_vault_setup');
    $r->post('/automator/api/vault/unlock')->to('automator#api_vault_unlock');
    $r->post('/automator/api/vault/lock')->to('automator#api_vault_lock');
    $r->websocket('/automator/ws/:history_id')->to('automator#ws');
}
