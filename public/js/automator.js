...
async function checkVaultStatus() {
    const data = await apiGet('/automator/api/status', 5000);
...
async function submitVault(event) {
    ...
    const endpoint = STATE.setup_required ? '/automator/api/vault/setup' : '/automator/api/vault/unlock';
...
async function lockVault() {
    const result = await apiPost('/automator/api/vault/lock', {});
    if (!result) return;
    if (ws) ws.close();
    showVaultGate(false);
}
...
async function refreshState(force = false) {
    ...
    const data = await apiGet(`/automator/api/state?${new URLSearchParams(getActiveFilters()).toString()}`, 6000);
    if (!data) return;
    if (data.locked) {
        showVaultGate(false);
        return;
...
async function savePlaybook(event) {
    ...
    const result = await apiPost('/automator/api/playbook/save', formData, 15000);
    if (!result) return;
    closePlaybookModal();
    await refreshState(true);
}
...
function deleteInventory(id) {
    ...
    showConfirmModal({
        ...
        hideCancel: true,
        onConfirm: async () => {
            await apiPost(`/automator/api/inventory/delete/${id}`, {});
            await refreshState(true);
        }
    });
}
...
async function saveInventory(event) {
    event.preventDefault();
    const result = await apiPost('/automator/api/inventory/save', new FormData(event.target), 15000);
    if (!result) return;
    closeInventoryModal();
    await refreshState(true);
}
...
async function saveSecret(event) {
    event.preventDefault();
    const result = await apiPost('/automator/api/secret/save', new FormData(event.target), 15000);
    if (!result) return;
    closeSecretModal();
    await refreshState(true);
}
...
function deletePlaybook(id) {
    ...
    showConfirmModal({
        ...
        hideCancel: true,
        onConfirm: async () => {
            await apiPost(`/automator/api/playbook/delete/${id}`, {});
            await refreshState(true);
        }
    });
}
...
async function startRun(event) {
    ...
    const result = await apiPost('/automator/api/run', {
...
async function loadHistoryPage(reset = false) {
    ...
    const data = await apiGet(`/automator/api/history?${params.toString()}`, 6000);
    if (!data || !data.success) return;
...
function connectWS(historyId) {
    if (ws) ws.close();
    ws = new WebSocket(`/automator/ws/${historyId}`);
...
function abortRun(id) {
    showConfirmModal({
        ...
        hideCancel: false,
        onConfirm: async () => {
            await apiPost(`/automator/api/abort/${id}`, {});
            await refreshState(true);
        }
    });
}
...
function globalAbort() {
    showConfirmModal({
        ...
        hideCancel: false,
        onConfirm: async () => {
            await apiPost('/automator/api/abort/all', {});
            await refreshState(true);
        }
    });
}
